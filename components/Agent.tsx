        ...
        {messages.length>0&&(
            <div className="transcript-border">
                <div className="transcript">
                    <p key={lastMessage} className={cn('transition-opacity duration-500 opacity-0','animate-fadeIn opactity-100')}>
                        {lastMessage}
                    </p>
                </div>
            </div>
        ...
        <div className="w-full flex justify-center m-5">
            {currentCallStatus !=='ACTIVE' ? (
                <button className="relative btn-call">
                    ...
                    <span>
                        {currentCallStatus === 'INACTIVE' || currentCallStatus === 'FINISHED' ? 'Call' : '. . .'}</span>
                </button>
            ) : (
